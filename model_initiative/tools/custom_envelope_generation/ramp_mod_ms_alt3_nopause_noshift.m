% ramp_mod_ms - echte Zeitverschiebung
%
...
%               2 to delay sustain
function out = ramp_mod_ms_alt3_nopause_noshift(len,fs,fc,parameters,dp,env_min,env_max,flank_type,is_reference)
...
% trim envelope if it is too long
if(size(ramp_l,1)>size(signal,2))
%    ramp_l(1:size(ramp_l,1)-size(signal,2))=[];
    ramp_l(size(ramp_l,1)+1-(size(ramp_l,1)-size(signal,2)):end)=[];
end
if(size(ramp_r,1)>size(signal,2))
%    ramp_r(1:size(ramp_r,1)-size(signal,2))=[];
    ramp_r(size(ramp_r,1)+1-(size(ramp_r,1)-size(signal,2)):end)=[];
end

% filename_ramp=sprintf('env_cond%d_tref1.mat',cputime)
% pause(1)
% envelope=[ramp_l ramp_r];
% save(filename_ramp,'envelope')
